...
//#endregion
//#region src/utils/no-think-fetch.ts
const VALID_DISABLE_THINKING_STRATEGIES = [
	false,
	"vllm",
	"deepseek",
	"dashscope",
...
function normalizeDisableThinking(raw) {
	...
	console.warn(`[memory-tdai] Unknown disableThinking strategy "${raw}", valid values: false, true, "vllm", "deepseek", "dashscope", "openai", "anthropic", "kimi", "gemini". Thinking will NOT be disabled.`);
	return false;
}
...
*/
function createNoThinkFetch(strategy = false) {
	if (strategy === false) return globalThis.fetch;
	const transform = STRATEGY_TRANSFORMERS[strategy];
	if (!transform) return globalThis.fetch;
	return (async (input, init) => {
		if (init && typeof init.body === "string") try {
...
function parseConfig(raw) {
	...
	return {
		...
		memoryCleanup,
		reindex: { approveChanges: bool(obj(c, "reindex"), "approveChanges") ?? false },
		report: {
...
*/
function initTimeModule(cfg, logger) {
...
*/
function formatLocalDateTime(d = /* @__PURE__ */ new Date()) {
	const parts = new Intl.DateTimeFormat("en-CA", {
		timeZone: _resolvedTz,
		year: "numeric",
		month: "2-digit",
...
*/
function startOfLocalDay(d = /* @__PURE__ */ new Date()) {
	...
	const utcMidnight = /* @__PURE__ */ new Date(`${dateStr}T00:00:00Z`);
	...
	if (localDay !== targetDay) offsetMinutes = offsetMinutes - 1440;
	return utcMidnight.getTime() - offsetMinutes * 60 * 1e3;
}
/**
...
function formatForLLM(input) {
	...
	const get = (type) => parts.find((p) => p.type === type).value;
	return `${`${get("year")}-${get("month")}-${get("day")}T${get("hour")}:${get("minute")}:${get("second")}`}${getUtcOffset(d)}`;
}
/**
...
function resolveTimeZone(cfg, logger) {
	if (!cfg || cfg === "system") return Intl.DateTimeFormat().resolvedOptions().timeZone;
...
*/
const TAG$30 = "[context-offload] [local-llm]";
/**
...
async function callLlm(config, opts, logger) {
	...
	logger?.info?.(`${TAG$30} ${label} >>> model=${config.model}, temp=${temperature}, timeout=${timeoutMs}ms, systemLen=${opts.systemPrompt.length}, userLen=${opts.userPrompt.length}`);
	const customFetch = opts.customFetch ?? (config.disableThinking ? createNoThinkFetch(config.disableThinking) : void 0);
	...
	try {
		...
		logger?.info?.(`${TAG$30} ${label} <<< ${elapsedMs}ms, output=${text.length} chars`);
		return text;
	} catch (err) {
		const elapsedMs = Date.now() - startMs;
		const errMsg = err instanceof Error ? err.message : String(err);
		logger?.error?.(`${TAG$30} ${label} FAILED (${elapsedMs}ms): ${errMsg}`);
		throw err;
	}
}
//#endregion
...
var LocalLlmClient = class {
	...
	constructor(cfg, logger) {
		...
		this.logger = logger;
		this.customFetch = cfg.disableThinking ? createNoThinkFetch(cfg.disableThinking) : void 0;
		logger?.info?.(`${TAG$29} Initialized: model=${cfg.model}, baseUrl=${cfg.baseUrl}`);
	}
	async l1Summarize(req) {
		...
		if (entries.length === 0) this.logger?.warn?.(`${TAG$29} L1: parsed 0 entries from LLM response (${raw.length} chars)`);
		return { entries };
	}
	async l15Judge(req) {
		...
		if (!result) {
			this.logger?.warn?.(`${TAG$29} L1.5: failed to parse judgment from LLM response (${raw.length} chars)`);
			return {
				taskCompleted: false,
	...
	async l2Generate(req) {
		...
		if (!result) {
			this.logger?.error?.(`${TAG$29} L2: failed to parse response (${raw.length} chars)`);
...
//#endregion
//#region src/offload/auth-profile-key.ts
/**
...
* of "config-tree only".
*/
const TAG$28 = "[context-offload] [auth-profile]";
...
*/
function resolveApiKeyFromAuthProfile(api, providerKey, logger, _loadSdkOverride) {
	try {
		...
		if (typeof ensureAuthProfileStore !== "function" || typeof listProfilesForProvider !== "function") return;
		const store = ensureAuthProfileStore(typeof resolveOpenClawAgentDir === "function" ? resolveOpenClawAgentDir() : void 0, { config: api.config });
		if (!store || typeof store !== "object") return void 0;
		const profileIds = listProfilesForProvider(store, providerKey);
		if (!Array.isArray(profileIds) || profileIds.length === 0) return void 0;
		const profiles = store.profiles ?? {};
		for (const id of profileIds) {
			const cred = profiles[id];
			if (cred?.type === "api_key" && typeof cred.key === "string" && cred.key.length > 0) {
				logger?.debug?.(`${TAG$28} Resolved api key for provider "${providerKey}" from profile "${id}"`);
				return cred.key;
			}
		}
		return;
	} catch (err) {
		logger?.debug?.(`${TAG$28} Auth-profile lookup unavailable for provider "${providerKey}": ${err instanceof Error ? err.message : String(err)}`);
		return;
	}
}
/**
...
* failure.
*/
function loadProviderAuthSdk() {
	try {
		return _require("openclaw/plugin-sdk/provider-auth");
	} catch {
		return;
	}
}
...
var OffloadStateManager = class OffloadStateManager {
	...
	*  L2 must wait for this to be true before triggering. */
	...
	confirmedOffloadIds = /* @__PURE__ */ new Set();
	/** Set of toolCallIds that were aggressively DELETED. */
	deletedOffloadIds = /* @__PURE__ */ new Set();
	/** Reconciliation retry counter */
	_reconcileRetries = /* @__PURE__ */ new Map();
	...
	*  remainingTokens: total tokens (incl sys) after aggressive compression. */
	...
	_pendingParams = /* @__PURE__ */ new Map();
	...
	_l1ChunkFailCounts = /* @__PURE__ */ new Map();
	...
	if (offloadConfig.mode === "backend" || offloadConfig.mode === "collect") if (!offloadConfig.backendUrl) logger.error(`[context-offload] mode=${offloadConfig.mode} but backendUrl not configured. L1/L1.5/L2/L4 disabled.`);
		...
		if (resolvedModelRef) {
			...
			if (baseUrl && apiKey) backendClient = new LocalLlmClient({
				...
				disableThinking: offloadConfig.disableThinking
			}, logger);
			else logger.error(`[context-offload] Local LLM mode failed: provider "${providerKey}" not found or missing baseUrl/apiKey in models.providers (or auth profiles). L1/L1.5/L2 disabled.`);
...
function localDayEndMs(year, month, day) {
	return startOfLocalDay(new Date(Date.UTC(year, month - 1, day + 1))) - 1;
}
...
function computeCutoffMsByLocalDay(nowMs, retentionDays) {
	const cutoffMs = startOfLocalDay(new Date(nowMs)) - (retentionDays - 1) * 24 * 60 * 60 * 1e3;
...
var MemoryPipelineManager = class {
	...
	*/
	...
	async runL2(sessionKey) {
		...
		else if (!state.last_extraction_updated_time) state.last_extraction_updated_time = (/* @__PURE__ */ new Date()).toISOString();
		await this.persistStates();
...
var VectorStore = class VectorStore {
	...
	*/
		...
		if (providerInfo) if (savedMeta) {
			...
			if (providerChanged || modelChanged || dimsChanged) {
				...
				if (l1Count === 0 && l0Count === 0) {
					this.logger?.info(`${TAG$21} Embedding config changed (${reindexReason}) but DB is empty (L1=0, L0=0) — nothing to re-embed; recreating vec tables, no reindex needed.`);
					this.dropVectorTables();
				} else {
					this.logger?.info(`${TAG$21} Embedding config changed (${reindexReason}). Vector tables left intact; rebuild deferred until the reindex runs (reindex.approveChanges).`);
					needsReindex = true;
			...
			if (l1Count > 0 || l0Count > 0) {
				this.logger?.info(`${TAG$21} No embedding_meta found but existing data exists (L1=${l1Count}, L0=${l0Count}). Rebuild deferred until the reindex runs...`);
				needsReindex = true;
				this.reindexPending = true;
				...
				this.logger?.debug?.(`${TAG$21} vec0 table dimension mismatch (existing=${existingVecDims}, required=${this.dimensions}) but DB is empty (L1=0, L0=0) — nothing to re-embed; recreating vec tables at the required width, no reindex needed.`);
				this.dropVectorTables();
			}
		}
		this.db.exec(`
    ...
    `);
		...
		if (this.dimensions > 0 && !this.reindexPending) this.db.exec(`
    ...
    `);
		if (this.dimensions > 0 && !this.reindexPending) this.stmtSearchVec = this.db.prepare(`
    ...
    `);
		if (this.dimensions > 0 && !this.reindexPending) this.stmtL0SearchVec = this.db.prepare(`
        SELECT record_id, distance
        FROM l0_vec
        WHERE embedding MATCH ?
          AND k = ?
    ...
    `);
      ...
      `);
		...
		if (providerInfo && !needsReindex) this.writeEmbeddingMeta({
	...
	*/
	readSchemaEpoch() {
		try {
			const row = this.db.prepare("SELECT value FROM embedding_meta WHERE key = ?").get("schema_epoch");
			if (!row) return 0;
	...
	writeSchemaEpoch(epoch) {
		try {
			this.db.prepare("INSERT INTO embedding_meta (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").run("schema_epoch", String(epoch));
		} catch {}
	}
	/**
	...
	*/
	vecEpochChanged() {
		...
		if (disk !== this.boundEpoch) {
			this.logger?.warn?.(`${TAG$21} Vec schema epoch advanced under this process (bound=${this.boundEpoch}, disk=${disk}) — another process rebuilt the vector tables. Skipping vec writes and freezing vector search until re-init; rows are persisted (metadata + FTS) and will be embedded on the next reindex pass.`);
	...
	*/
	markEmbeddingCurrent(providerInfo) {
		if (!providerInfo) return;
		const dims = providerInfo.dimensions ?? this.dimensions;
		if (providerInfo.dimensions != null && providerInfo.dimensions !== this.dimensions) this.logger?.warn(`${TAG$21} markEmbeddingCurrent: provider reported ${providerInfo.dimensions} dims but vector tables are ${this.dimensions} — recording table width. Check embedding.dimensions in config.`);
	...
	*/
	rebuildVecTables(providerInfo) {
		if (this.degraded) {
			this.logger?.warn(`${TAG$21} rebuildVecTables skipped: VectorStore is in degraded mode`);
			return false;
		}
		if (!this.dimensions || this.dimensions <= 0) {
			this.logger?.warn(`${TAG$21} rebuildVecTables refused: dimensions=${this.dimensions}`);
			return false;
		}
		try {
			this.dropVectorTables();
      ...
      `);
			...
			const message = err instanceof Error ? err.message : String(err);
			this.logger?.error(`${TAG$21} rebuildVecTables failed: ${message}. VectorStore entering degraded mode.`);
			this.degraded = true;
			this.vecTablesReady = false;
			return false;
		}
		...
		try {
			...
			for (const { record_id, content, updated_time } of l1Rows) {
				try {
					...
					l1Failed++;
					this.logger?.warn?.(`${TAG$21} reindex L1 skip ${record_id} (${l1Failed}): ${err instanceof Error ? err.message : String(err)}`);
				}
			...
			for (const { record_id, message_text, recorded_at } of l0Rows) {
				try {
					...
					l0Failed++;
					this.logger?.warn?.(`${TAG$21} reindex L0 skip ${record_id} (${l0Failed}): ${err instanceof Error ? err.message : String(err)}`);
				}
				onProgress?.(l0Done, l0Failed, l0Rows.length, "L0");
			}
			this.logger?.info(`${TAG$21} Reindex done: L1=${l1Done}/${l1Rows.length} (${l1Failed} failed), L0=${l0Done}/${l0Rows.length} (${l0Failed} failed)`);
			return {
				l1Count: l1Done,
				l0Count: l0Done,
				l1Failed,
...
*/
const LLM_RETRY_LIMIT$1 = 1;
/** Appended to the user prompt on a retry, steering the model back to pure JSON. */
const RETRY_REPAIR_HINT$1 = "\n\n[SYSTEM] Your previous reply was empty or not valid JSON, so it could not be parsed. Return ONLY a single valid JSON array matching the requested schema (array of {record_id, action, target_ids, merged_content, merged_type, merged_priority, merged_timestamps}), with no markdown fences, no prose, and no trailing text.";
/**
...
async function runLlmJudgment(matches, memories, config, logger, model, llmRunner) {
	...
	for (let attempt = 1; attempt <= 1 + LLM_RETRY_LIMIT$1; attempt++) try {
		const userPrompt = formatBatchConflictPrompt(matches) + (attempt > 1 ? RETRY_REPAIR_HINT$1 : "");
		let result;
		...
		if (attempt <= LLM_RETRY_LIMIT$1) {
			logger?.info?.(`${TAG$20} Empty or unparseable conflict-detection response (attempt ${attempt}/${1 + LLM_RETRY_LIMIT$1}, rawLen=${result.length}) — retrying with repair hint.`);
			await new Promise((r) => setTimeout(r, 2e3 * attempt));
		} else {
			logger?.warn?.(`${TAG$20} Conflict detection unparseable after ${attempt} attempts (rawLen=${result.length}) — defaulting all to store.`);
...
function parseBatchResult(raw, memories, logger) {
	try {
		...
		if (!arrayMatch) {
			logger?.debug?.(`${TAG$20} No JSON array found in conflict detection response (rawLen=${cleaned.length})`);
			return null;
		}
		const sanitized = sanitizeJsonForParse(arrayMatch[0]);
		const parsed = JSON.parse(sanitized);
		if (!Array.isArray(parsed)) {
			logger?.debug?.(`${TAG$20} Conflict detection response is not an array (rawLen=${cleaned.length})`);
			return null;
		}
		const decisions = [];
		const validActions = [
		...
		logger?.debug?.(`${TAG$20} Failed to parse conflict detection result: ${err instanceof Error ? err.message : String(err)}`);
		return null;
	}
}
/**
...
*/
function normalizeLlmText(result) {
...
const RETRY_REPAIR_HINT = "\n\n[SYSTEM] Your previous reply was empty or not valid JSON, so it could not be parsed. Return ONLY a single valid JSON array matching the requested schema (array of {scene_name, message_ids, memories}), with no markdown fences, no prose, and no trailing text.";
/**
...
async function callLlmExtractionWithRetry(params) {
	...
	for (let attempt = 1; attempt <= 1 + LLM_RETRY_LIMIT; attempt++) {
		...
		if (attempt <= LLM_RETRY_LIMIT) {
			logger?.info?.(`${TAG$18} Empty or malformed extraction response (attempt ${attempt}/${1 + LLM_RETRY_LIMIT}, rawLen=${raw.length}) — retrying with repair hint. (If retries fail, the pipeline re-presents this batch on later cycles.)`);
			await new Promise((r) => setTimeout(r, 2e3 * attempt));
			continue;
		}
		logger?.info?.(`${TAG$18} Extraction unparseable after ${attempt} attempts — deferring. The pipeline re-presents this batch on later cycles, so nothing is lost.`);
		return [];
	}
	return [];
}
...
function parseExtractionResult(raw, logger) {
	...
	try {
		...
		const arrayMatch = cleaned.match(/\[[\s\S]*\]/);
		if (!arrayMatch) {
			logger?.info?.(`${TAG$18} No JSON array found in extraction response (will retry)`);
			const rawPreview = text.slice(0, 2048);
			logger?.info?.(`${TAG$18} [l1-debug] NO_JSON taskId=l1-extraction, rawLen=${text.length}, cleanedLen=${cleaned.length}, rawFull=${JSON.stringify(rawPreview)}${text.length > 2048 ? `…(+${text.length - 2048})` : ""}`);
			return null;
		}
		const sanitized = sanitizeJsonForParse(arrayMatch[0]);
		const parsed = JSON.parse(sanitized);
		...
		const message = err instanceof Error ? err.message : String(err);
		logger?.warn?.(`${TAG$18} Failed to parse extraction result: ${message}`);
		const rawPreview = text.slice(0, 2048);
		logger?.warn?.(`${TAG$18} [l1-debug] PARSE_FAIL taskId=l1-extraction, rawLen=${text.length}, rawPreview=${JSON.stringify(rawPreview)}${text.length > 2048 ? `…(+${text.length - 2048})` : ""}`);
		return null;
	}
}
/**
...
var TcvdbMemoryStore = class TcvdbMemoryStore {
	...
	*/
	...
	markEmbeddingCurrent(_providerInfo) {}
	isFtsAvailable() {
...
function truncateEmbeddingInputs(texts, maxInputChars, logger) {
	...
	return texts.map((text) => {
		...
		logger?.warn?.(`${TAG$15} Input truncated from ${text.length} to ${maxInputChars} chars (maxInputChars limit)`);
...
var ZeroEntropyEmbeddingService = class {
	...
	logger;
	constructor(config, logger) {
		if (!config.apiKey) throw new Error("ZeroEntropyEmbeddingService: apiKey is required");
		if (!config.baseUrl) throw new Error("ZeroEntropyEmbeddingService: baseUrl is required");
		if (!config.model) throw new Error("ZeroEntropyEmbeddingService: model is required");
		if (!config.dimensions || config.dimensions <= 0) throw new Error("ZeroEntropyEmbeddingService: dimensions is required (must be a positive integer)");
		this.baseUrl = config.baseUrl.replace(/\/+$/, "");
		this.apiKey = config.apiKey;
		this.model = config.model;
		this.dims = config.dimensions;
	...
	async _callApi(texts, timeoutOverride) {
		...
		if (this.sendDimensions) body.dimensions = this.dims;
		const json = await postEmbeddingRequest({
			fetchUrl: `${this.baseUrl}/models/embed`,
			headers: {
				"Content-Type": "application/json",
				Authorization: `Bearer ${this.apiKey}`
			},
		...
		if (!json.results || !Array.isArray(json.results)) throw new Error("ZeroEntropy embedding API returned unexpected format: missing 'results' array");
...
function createEmbeddingService(config, logger) {
	if (config && config.provider === "zeroentropy" && "apiKey" in config && config.apiKey) {
		logger?.debug?.(`${TAG$15} Using ZeroEntropy embedding (model=${config.model})`);
...
function parseSceneBlock(raw, filename) {
	...
	return {
		...
		meta: {
			...
			project: extractMetaField(metaBlock, "project") || void 0
		},
		content
	};
}
	...
	try {
		...
		for (const item of parsed) {
			...
			entries.push({
				...
				project: typeof item.project === "string" && item.project ? item.project : void 0
			});
		}
		return entries;
	} catch {
...
function buildPersonaPrompt(params) {
	...
	return {
		...
		userPrompt: `**Output language**: \`persona.md\` headings and body text must use the dominant language of the changed scene content below. For English scene content, use English persona headings.
...
var StandaloneLLMRunner = class {
	...
	constructor(opts) {
		...
		this.logger = opts.logger;
		this.customFetch = opts.config.disableThinking ? createNoThinkFetch(opts.config.disableThinking) : void 0;
	}
	async run(params) {
		const runStartMs = Date.now();
...
function deriveAgentIdFromSessionKey(sessionKey) {
	if (!sessionKey || !sessionKey.startsWith("pi:")) return void 0;
	const rest = sessionKey.slice(3);
	const idx = rest.indexOf(":");
	return (idx === -1 ? rest : rest.slice(0, idx)) || void 0;
}
/**
...
*/
function recordVisibleToAgent(recordSessionKey, agentId) {
...
*/
function sceneVisibleToAgent(project, agentId) {
	if (!agentId) return true;
...
async function performAutoRecallInner(params) {
	...
	const l1AgentScope = cfg.recall.scopeL1 === "agent" ? agentId : void 0;
	...
	if (!userText || userText.length === 0) logger?.debug?.(`${TAG$4} User text empty/undefined, skipping memory search (persona/scene still injected)`);
		...
		const searchResult = await searchMemories(userText, pluginDataDir, cfg, logger, effectiveStrategy, vectorStore, embeddingService, l1AgentScope);
...
*/
async function searchMemories(userText, pluginDataDir, cfg, logger, strategy, vectorStore, embeddingService, agentScope) {
	const emptyResult = {
		lines: [],
		timing: {
			ftsMs: 0,
...
async function searchByKeyword(userText, _pluginDataDir, maxResults, threshold, logger, vectorStore, agentScope) {
	if (vectorStore?.isFtsAvailable()) {
		...
		if (ftsQuery) {
			...
			const ftsResults = agentScope ? allResults.filter((r) => recordVisibleToAgent(r.session_key, agentScope)) : allResults;
...
async function searchByEmbedding(userText, maxResults, threshold, vectorStore, embeddingService, logger, embeddingCallOpts, agentScope) {
	...
	const allVecResults = await vectorStore.searchL1Vector(queryEmbedding, maxResults * 2);
	const vecResults = agentScope ? allVecResults.filter((r) => recordVisibleToAgent(r.session_key, agentScope)) : allVecResults;
...
*/
async function searchHybrid(userText, _pluginDataDir, maxResults, _threshold, vectorStore, embeddingService, logger, embeddingCallOpts, agentScope) {
	...
	const [keywordResult, embeddingResult] = await Promise.all([(async () => {
			...
			if (vectorStore.isFtsAvailable()) {
				...
				if (ftsQuery) {
					...
					if (ftsResults.length > 0) {
						...
						return {
							records: ftsResults.filter((r) => recordVisibleToAgent(r.session_key, agentScope)).map((r) => ({
								record: {
									id: r.record_id,
									content: r.content,
									type: r.type,
		...
		try {
			...
			const results = agentScope ? allResults.filter((r) => recordVisibleToAgent(r.session_key, agentScope)) : allResults;
...
function truncateRecallLine(line, maxChars) {
	...
	if (maxChars <= 60) return cps.slice(0, maxChars).join("");
	return `${cps.slice(0, maxChars - 60).join("").trimEnd()}${RECALL_TRUNCATION_SUFFIX}`;
}
/**
...
//#endregion
//#region src/core/store/reindex-lock.ts
/**
...
const DEFAULT_MAX_AGE_MS = 1800 * 1e3;
var ReindexLock = class {
	lockPath;
	maxAgeMs;
	debug;
	ownerNonce;
	...
	readOwner() {
		try {
			return JSON.parse(fsSync.readFileSync(this.lockPath, "utf8"));
		} catch {
			return null;
		}
	}
	...
	createLockFile() {
		...
		try {
			...
			fsSync.linkSync(tmp, this.lockPath);
			this.held = true;
			return true;
		} catch {
			return false;
	...
	*/
	stealStale() {
		try {
			const fd = fsSync.openSync(this.lockPath, "w");
			fsSync.writeSync(fd, JSON.stringify({
				nonce: this.ownerNonce,
				pid: process.pid,
				ts: Date.now()
			}));
		...
		if (this.readOwner()?.nonce !== this.ownerNonce) return false;
		this.held = true;
		return true;
	}
	/**
	...
	*/
	tryAcquire() {
		...
		if (age > this.maxAgeMs) {
			this.debug?.(`reindex lock stale (age ${Math.round(age / 6e4)}min) — stealing`);
	...
	*/
	heartbeat() {
		if (!this.held) return;
		try {
			if (this.readOwner()?.nonce !== this.ownerNonce) return;
			const fd = fsSync.openSync(this.lockPath, "w");
			fsSync.writeSync(fd, JSON.stringify({
				nonce: this.ownerNonce,
				pid: process.pid,
	...
	*/
	release() {
		if (!this.held) return;
		try {
			if (this.readOwner()?.nonce === this.ownerNonce) fsSync.unlinkSync(this.lockPath);
		} catch {}
		this.held = false;
	}
};
		...
		try {
			...
			if (stores.needsReindex && this.vectorStore && this.embeddingService) {
				if (!this.cfg.reindex.approveChanges) {
					this.logger?.warn(`${TAG$1} Embedding config change detected (${stores.reindexReason ?? "provider/model/dimensions"}) but reindex is NOT auto-approved. Vector tables are frozen (intact, not rebuilt) and vector search is disabled until you approve. To approve, set "reindex": { "approveChanges": true } in the memory config and restart, or run the explicit reindex tool/script (scripts/reindex-now.ts). Reverting the embedding config restores the previous index untouched. Keyword/FTS recall is unaffected.`);
					return;
				}
				const vs = this.vectorStore;
				const es = this.embeddingService;
				const reason = stores.reindexReason ?? "embedding config changed";
				this.logger?.info(`${TAG$1} ${reason}; reindex APPROVED (reindex.approveChanges=true); rebuilding vector tables and re-embedding all stored texts in the background (L1 + L0 through ${this.cfg.embedding.provider}/${this.cfg.embedding.model ?? es.getProviderInfo().model})...`);
				(async () => {
					const reindexLock = new ReindexLock({
						dir: this.dataDir,
						debug: (msg) => this.logger?.info?.(`${TAG$1} ${msg}`)
					});
					if (!reindexLock.tryAcquire()) {
						this.logger?.info?.(`${TAG$1} Reindex lock held by another live process — skipping (they will mark embedding current).`);
						return;
					}
					const heartbeat = setInterval(() => reindexLock.heartbeat(), 6e4);
					try {
						if (!es.isReady()) {
							this.logger?.info?.(`${TAG$1} Waiting for embedding service readiness before reindex...`);
							es.startWarmup();
							const readyAt = Date.now() + 300 * 1e3;
							while (!es.isReady()) {
								if (Date.now() > readyAt) {
									this.logger?.warn?.(`${TAG$1} Embedding service not ready after 5min — aborting reindex. Tables left frozen (nothing destroyed); meta stays stale; next boot will retry.`);
									return;
								}
						...
						if (!vs.rebuildVecTables?.(es.getProviderInfo())) {
							this.logger?.warn?.(`${TAG$1} Vector table rebuild refused/failed — aborting reindex. Tables left frozen (nothing destroyed); meta stays stale; next boot will retry.`);
							return;
						}
						const result = await vs.reindexAll(async (text) => es.embed(text), (succeeded, failed, totalCount, layer) => {
							if (succeeded === 1 || succeeded % 500 === 0 || succeeded + failed === totalCount) this.logger?.info?.(`${TAG$1} reindex progress: ${layer} ${succeeded} ok / ${failed} failed / ${totalCount} total`);
						});
						...
						if (result.l1Failed === 0 && result.l0Failed === 0 && done === total) {
							...
							this.logger?.info?.(`${TAG$1} Background reindex complete: L1=${result.l1Count}/${result.l1Total}, L0=${result.l0Count}/${result.l0Total}; embedding marked current.`);
						} else this.logger?.warn?.(`${TAG$1} Reindex incomplete (L1 ${result.l1Count}/${result.l1Total} done, ${result.l1Failed} failed; L0 ${result.l0Count}/${result.l0Total} done, ${result.l0Failed} failed). Embedding meta left stale so the next boot retries. Keyword recall unaffected.`);
					} catch (err) {
						this.logger?.warn(`${TAG$1} Background reindex failed (non-fatal; keyword recall unaffected): ${err instanceof Error ? err.message : String(err)}`);
					} finally {
